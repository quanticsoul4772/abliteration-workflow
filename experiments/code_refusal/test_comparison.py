...
def test_model(
    model, tokenizer, prompts: list, markers: list, system_prompt: str, model_name: str
):
    ...
    for i, prompt in enumerate(prompts):
        ...
        results.append({"prompt": prompt, "response": response, "refused": is_refusal})
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Compare original vs abliterated model"
    )
    parser.add_argument(
        "--original",
        type=str,
    ...
    parser.add_argument(
        "--abliterated",
        type=str,
        default="./experiments/code_refusal/Qwen2.5-7B-Instruct-heretic",
    ...
    parser.add_argument(
        "--output",
        type=str,
        default="./experiments/code_refusal/comparison_results.json",
